...
state_names_df <- fips_codes_df |> filter(`Summary Level` == 40) |> mutate(`State` = `Area Name (including legal/statistical area description)`) |> select(`State Code (FIPS)`, `State`) 

fips_codes_cleaned_df <- fips_codes_df |> filter(`Summary Level` == 50) |> mutate(`County` = `Area Name (including legal/statistical area description)`) |> select(!(`Consolidtated City Code (FIPS)`:`Area Name (including legal/statistical area description)`))
